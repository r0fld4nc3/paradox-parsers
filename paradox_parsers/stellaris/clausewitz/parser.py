...
class Parser:
    ...
    def _parse_scalar(self) -> Atom:
        t = self._next()
        if t.kind == TokenKind.ATOM:
            return Atom(text=t.text, quoted=False)
        if t.kind == TokenKind.STRING:
            return Atom(text=t.text, quoted=True)
    ...
    def parse_into_sql(self, sql_f: SqliteSink) -> int:
        ...
        while True:
            ...
            key = self._parse_key_atom()
            self._expect(TokenKind.EQUALS)
            self._parse_value_as_assign(sql_f, key)

        sql_f.end_block()
        return root_block_id

    def _parse_value_as_assign(self, sql_f: SqliteSink, key_text: str) -> None:
        t = self._peek()

        if t.kind == TokenKind.LBRACE:
            child = self._parse_block(sql_f)
            sql_f.assign_block(key_text, child)
        else:
            atom = self._parse_scalar()
            sql_f.assign_scalar(key_text, atom.text, atom.quoted)

    def _parse_value_item(self, sql_f: SqliteSink) -> None:
        t = self._peek()
        if t.kind == TokenKind.LBRACE:
            child = self._parse_block(sql_f)
            sql_f.value_block(child)
        else:
            atom = self._parse_scalar()
            sql_f.value_scalar(atom.text, atom.quoted)
